<template>
    <f7-page infinite :infinite-distance="200" :infinite-preloader="showPreloader" @infinite="loadBooks"
             ptr @ptr:refresh="refreshBooks" :ptr-preloader="showPtrPreloader">
        <f7-block-title large>{{ category.name }}</f7-block-title>
        <f7-list media-list>
          ...
          >
            <f7-page :style="applybackground(b.image)">
              ...
              >
                ...
                >
                  <div :key="asset.id" v-for="asset in getAssets(page.book_id,page.id)" :style="applyAssetStyle(asset)">
                    ...
                    <f7-card v-else-if="asset.type=='link'">
                      <f7-link
                        ...
                        external
                        target="_system"
                        @click='window.open(`${asset.content}`, "_system"); return false;'>{{asset.content}}</f7-link>
                    </f7-card>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getCategory() {
        const self = this;
        self.$crequest().get(`/category/${self.categoryId}`).then((r) => {
          self.category = r.data.data;
          self.getBooks();
        }).catch(() => {
      ...
      async getBooks(done = false) {
        const self = this;
        self.offset = self.limit * self.lastPage;
        ...
        self.$crequest().get(`/category/${self.categoryId}/book?${params}`).then((r) => {
          if(r.data.total_count > 0) {
            self['category/addBooks']({
              categoryId: Number(self.categoryId),
              books: r.data.data,
            });
          ...
          setTimeout(function () {
            ...
            if(self.lastPage === 0) {
              self.bottomMessage = 'Goruntulenecek yayin yok.'
            } else {
              self.bottomMessage = 'Goruntulenek yeni yayin yok.';
            }
            self.showPtrPreloader = false;
            if(done){
              done();
    ...
    applyAssetStyle(asset){
      var assetResult={'position':"relative"};
          ...
          if(asset.type === "background") {
            ...
            if(asset.data !== null && typeof asset.data['css'] !== 'undefined' && (asset.data['css'].backgroundPosition !== null)) {
              assetResult["background-position"] = asset.data.css.backgroundPosition;
              assetResult["background-size"] = asset.data.css.backgroundSize;             
            }             
          } else {
            assetResult["z-index"] = ((asset && asset.data !== null && asset.data.css !== null && asset.data.css.zIndex !== null) ? asset.data.css.zIndex : 0);
              assetResult.width = ((asset && asset.data !== null && asset.data.css !== null) && asset.data.css.width !== 'null' ? `${asset.data.css.width}%` : 0);
              assetResult.height = ((asset && asset.data !== null && asset.data.css !== null) && asset.data.css.height !== 'null' ? `${asset.data.css.height}%` : 0);
              assetResult.top = ((asset && asset.data !== null && asset.data.css !== null) && asset.data.css.top !== 'null' ? `${asset.data.css.top}%` : 0);
              assetResult.left = ((asset && asset.data !== null && asset.data.css !== null) && asset.data.css.left !== 'null' ? `${asset.data.css.left}%` : 0);
              assetResult['text-align'] = (typeof asset.data.css.fontAlign !== 'undefined' && asset.data.css.fontAlign !== null ? asset.data.css.fontAlign : 'left');
              assetResult.color=(typeof asset.data.css.color !== 'undefined' && asset.data.css.color !== null ? asset.data.css.color : '#000000');
          }      
    ...
    applybackground(img){
      ...
      result["backgroundImage"]="url("+this.$crequest().genAuthStaticCDNUrl(`${img}`)+"]";
      return result;
    },
    openPopup(key) {
